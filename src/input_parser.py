    ...
    with open(output_path, 'w') as output_file:
        ...
        for j in range(J):
            ...
            for i in range(N):
                ...
                print(l, file=output_file, end='')
                print('', file=output_file) if i < N - 1 or j < J - 1 else print(';', file=output_file)
        print(f'\nparam last_shift :=', file=output_file)
        for i in range(N):
            print(f'n{i + 1} ', file=output_file, end='')
            last_shift = Data['last_shift'][i][1:]
            if last_shift == []:  # pusty wektor oznacza, że nikt nie pracował
                last_shift_value = ' 0'
            else:  # jeśli są jakieś dane
                last_shift_value = ' ' + str(last_shift[0])
            print(last_shift_value, file=output_file, end='')
            print('', file=output_file) if i < N - 1 else print(';', file=output_file)
    return N, J, K
